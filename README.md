...
- Every file request goes through the Next server and an authorized Convex HTTP action. No public storage URL, storage ID, bridge secret or host hash is sent to the browser. A downloaded copy cannot be revoked.
...
An upload reserves quota before storage and releases it idempotently after failure. The HTTP action deletes a newly stored file if finalization fails. A provider process termination between storage and database finalization could still leave an unreferenced private file. Before paid use, establish an operator reconciliation check on the **new isolated deployment's** storage inventory; investigate unreferenced files older than one hour, verify they are outside active reservations, and remove them using the provider dashboard. Do not blindly scan/delete storage in the legacy project. Provider backups and disaster recovery retention are separate from live file deletion and must match the written event terms. This repository does not promise immediate deletion from provider backups.
